...
Any yield gap between the 2 regions collapses once works are included. Foreign buyers should compare net yield after restoration capex, IMU at 0.86% to 1.06% of cadastral value and 21% cedolare secca, rather than gross yield on an asking price that assumes a finished property.
...
Puglia typically suits foreign buyers restoring trulli or masserie who will manage a permit chain; Sicily suits those working with baroque or coastal stock where seismic compliance and heritage clearance dominate. Both are works markets, so the buyer profile in each is patient rather than yield-driven.
