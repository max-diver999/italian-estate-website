...
| Dimension | Elective residence | Investor Visa |
|---|---|---|
| Basis | Stable passive income | Qualifying investment from €250,000 |
| Right to work in Italy | No | Yes |
| Property role | Accommodation evidence only | Accommodation evidence only |
| Ongoing condition | Income must continue | Investment must be held |
...
- Both permits require Italy to be a genuine residence, not a mailbox: prolonged absence puts renewal at risk.
- Becoming Italian tax resident follows the 183-day test, which is separate from the immigration permit.
...
- Dependants' permits derive from the principal's status on both routes, so 1 lapse affects the household.
- Accommodation evidence must cover the whole family, not the applicant alone.
- Every civil-status document issued abroad needs a certified Italian translation.
...
Neither residence route depends on the property, so a purchase alongside an application should be verified exactly as any other would be. That means the visura catastale, the conformità position and the condominium's finances, none of which the visa process examines on a buyer's behalf.

The cross-check is an avvocato of your own choosing examining the visura catastale and conformità edilizia ahead of the caparra. Model 15% gross-to-net spread with commercialista before compromesso deposit authorization.
...
- Neither visa route depends on the property, so the purchase should stand on its own merits.
- Budget an all-in cost of 10% to 15% on a non-resident second home regardless of which permit is held.
- IMU applies at 0.86% to 1.06% of cadastral value with no primary-residence relief on a second home.
