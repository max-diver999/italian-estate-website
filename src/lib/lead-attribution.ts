import {
  detectExternalAiSource as detectReferenceAiSource,
  type ExternalAiSource as KnownExternalAiSource,
} from './external-ai-sources';

export type ExternalAiSource = KnownExternalAiSource | '';
...
const STORAGE_KEYS = {
  landingPage: 'ie_landing_page',
  firstReferrer: 'ie_first_referrer',
  sessionId: 'ie_session_id',
  lastCta: 'ie_last_cta',
} as const;

export function detectExternalAiSource(referrer: string): ExternalAiSource {
  return detectReferenceAiSource(referrer) ?? '';
}
