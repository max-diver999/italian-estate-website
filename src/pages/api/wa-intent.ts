...
const LEAD_FROM_EMAIL =
  import.meta.env.LEAD_FROM_EMAIL || `Italian Estate <${SITE.email}>`;
...
function normalizeSiteUrl(value: unknown): string {
  ...
  try {
    ...
    if (host !== 'italian-estate.com' && !host.endsWith('.vercel.app')) {
      return '';
    }
    return url.href;
  } catch {
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const lines = [
      '💬 <b>WhatsApp click | italian-estate.com</b>',
    ...
    const plainLines = [
      'WhatsApp click | italian-estate.com',
      `Ref: ${refCode}`,
    ...
    try {
      await sendLeadNotifyEmail({
        subject: `WhatsApp click — italian-estate.com (${refCode})`,
        htmlBody: plainLines,
