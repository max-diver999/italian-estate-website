...
const LEAD_FROM_EMAIL = import.meta.env.LEAD_FROM_EMAIL || `Italian Estate <${SITE.email}>`;
...
async function sendAutoReply(to: string, name: string, context: string) {
  ...
  const topic = context ? escapeHtml(context) : 'your Italy property enquiry';
    ...
    if (!isHealthcheck) {
      try {
        await sendLeadNotifyEmail({
          subject: 'New lead — italian-estate',
          htmlBody: lines,
          from: LEAD_FROM_EMAIL,
        });
      } catch (err) {
