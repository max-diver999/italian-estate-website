...
import { buildCorpusIndex, analyseDoc, plainText, words, sentences, figureRegex, normaliseFigure } from './corpus-signals.mjs';
...
/**
 * Where each new penalty starts paying, and where the measurement put it.
 *
 * Read these as "the most a hand-written article on this site actually
 * produced", not as dials. The twelve articles in the good set measured:
 * template shape share 0.0000 (all twelve exactly zero, max 0.0004 at a six-word
 * window), one echoing section at most, definition-frame openers at most 0.14 of
 * sections, and 1.58 qualifier piles per 1000 words. Each constant sits at or
 * just above that, so the rubric charges for the defect and not for the genre.
 *
 * The gates are set well clear of the middle set, which is the honest test of a
 * cap: semi-automatic prose reached 0.022 shape share and 0.14 frame openers, so
 * capping at 0.05 and 0.40 cannot fire on prose that a person has been through.
 */
export const TEMPLATE_SHAPE_FREE = 0.005;
export const TEMPLATE_SHAPE_GATE = 0.05;
export const DEFINITION_FRAME_FREE = 0.15;
export const DEFINITION_FRAME_GATE = 0.40;
export const HEADING_ECHO_FREE = 1;
...
export function scoreDocument(docId, index, { requireRegistry = true } = {}) {
  ...
  // Every threshold below is placed at the hand-written set's observed maximum,
  // so an article written the way the good set was written pays nothing. That is
  // the only defensible place to put them: a threshold chosen to make a number
  // pass is not a measurement, and a threshold below what good writing produces
  // is a tax on writing.
  if (corpus.templateShapeShare > TEMPLATE_SHAPE_FREE) {
    add((corpus.templateShapeShare - TEMPLATE_SHAPE_FREE) * 500, 'template-family',
      `${(corpus.templateShapeShare * 100).toFixed(1)}% of this article's sentence shapes recur in 3+ other articles (${corpus.sharedSkeletons.length} of ${corpus.templateShapeTotal}), e.g. "${corpus.sharedSkeletons[0]?.example}"`);
  }
  if (signals.definitionFrame.share > DEFINITION_FRAME_FREE) {
    add((signals.definitionFrame.share - DEFINITION_FRAME_FREE) * 60, 'definition-frame',
      `${signals.definitionFrame.count} of ${signals.definitionFrame.sections} sections open by defining their own title rather than answering it, e.g. "${signals.definitionFrame.example}"`);
  }
  // Qualifier pile-ups are measured and reported but deliberately not scored.
  // On the first, over-inclusive labelling they looked adopted (AUC 0.885); on
  // the final sets they fall to 0.841, below the bar every other rule here
  // cleared. Wiring them in anyway changes separation by 0.1 of a point and
  // costs the semi-automatic middle 0.9, which is the signature of a rule that
  // taxes prose rather than catching a template. It stays in --explain because
  // an editor rewriting a page still wants to see them.
  ...
  // preliminare trascritto?" answered by naming the thing has nowhere else to
  // start. Two is where the machine files live (mean 3.7).
  for (const e of signals.headingEchoes.slice(HEADING_ECHO_FREE)) {
  ...
  if (corpus.templateShapeShare > TEMPLATE_SHAPE_GATE) {
    gates.push({ code: 'template-corpus', cap: 25,
      detail: `${(corpus.templateShapeShare * 100).toFixed(1)}% of this article's sentence shapes recur in three or more other articles` });
  }
  if (signals.definitionFrame.share >= DEFINITION_FRAME_GATE) {
    gates.push({ code: 'definition-frame', cap: 25,
      detail: `${signals.definitionFrame.count} of ${signals.definitionFrame.sections} section openers define the heading instead of answering it` });
  }
 ...
 *
 * The index is memoised on the peer list because the caller scores every file
 * in a set against the same peers, and rebuilding it per file is quadratic in
 * the size of the set. On the reference site the garbage set was 59 files and
 * nobody noticed; here it is 252, which is 63,504 document preparations and
 * turns a calibration run into a coffee break. Nothing about the arithmetic
 * changes: the same peer list always produces the same index, so the cached
 * one is the one that would have been built.
 */
const calibrationIndexCache = new Map();

export async function scoreFileForCalibration(file, peers) {
  const key = peers.join('\n');
  let index = calibrationIndexCache.get(key);
  if (!index) {
    index = loadCorpus(peers);
    calibrationIndexCache.set(key, index);
  }
