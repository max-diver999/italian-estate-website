...
export const SHAPE_N = 8;

/**
 * Every shape a sentence contains, not just the one it starts with.
 *
 * The prefix skeleton above assumes the template's fixed part comes first, which
 * was true of the corpus it was written for. The generator here does the
 * opposite: it opens with the page's own subject and puts the fixed part in the
 * middle, so "ARIA Ex Macello typically means a 150,000 sqm district" and
 * "Alba overview means the Barolo gateway combines truffle tourism" share a
 * frame that no prefix comparison can see. Measured on the labelled sets, the
 * prefix version found 0 to 3 shared skeletons in files that were entirely
 * template; the sliding version finds a median of 110 and scores every one of
 * the twelve hand-written articles at exactly zero.
 *
 * Eight words is the window. Six separates marginally better (AUC 1.000 against
 * 0.998) and was rejected anyway: at six words a shape starts colliding on
 * ordinary English, and a rule that caps a score has to be wrong less often than
 * it is right. Ten and twelve both separate less (0.986, 0.960).
 */
export function sentenceShapes(sentence) {
  const shape = sentence
    ...
    .filter(Boolean);
  const out = [];
  for (let i = 0; i + SHAPE_N <= shape.length; i += 1) out.push(shape.slice(i, i + SHAPE_N).join(' '));
  return out;
}
...
export function duplicationText(raw) {
  ...
  //
  // Two prop shapes carry prose here, and only one of them is a JSX attribute.
  // This corpus passes its FAQ as an array of object literals -- `<FaqBlock
  // items={[{ question: "...", answer: "..." }]}>` -- so the attribute form
  // alone left 94,318 words, 12.2% of the corpus, outside the detector on 264
  // of 272 files. That is the same hole the red team walked through, arrived at
  // by convention rather than by attack, so both shapes are recovered.
  const attrText = (raw.match(/(?:text|answer|question|description|title)\s*=\s*"([^"]{40,})"/g) || [])
    .map((m) => m.replace(/^[^"]*"/, '').replace(/"$/, ''));
  const objectText = (raw.match(/\b(?:text|answer|question|description|title|label)\s*:\s*"((?:[^"\\]|\\.){40,}?)"/g) || [])
    .map((m) => m.replace(/^[^"]*"/, '').replace(/"$/, ''));
  const componentText = [...attrText, ...objectText].join(' ');
...
/**
 * Figure phrases: "€620,000", "7.5%", "14 business days" — the unit matters, the
 * bare digit does not.
 *
 * The pattern lived in three places and every consumer has to see the same
 * figures, so it is defined once here. Three copies is how a port half-works:
 * saturation detection notices a stamped figure while provenance does not ask
 * for a source for it, and neither says anything is wrong.
 *
 * Two spellings beyond the obvious one are matched, because this corpus uses
 * them and a detector that cannot read them is blind on exactly the text it was
 * built for. Percentages appear as "10 percent" as well as "10%", and amounts
 * as "1,500 EUR" as well as "€1,500" — 6.9% and 5.2% of all figures in the
 * machine snapshot respectively. The cost of missing them is not marginal:
 * "10 to 15 percent" is stamped across 131 files there and was invisible, so
 * the article carrying it collected no stamped-figure penalty at all.
 *
 * Spellings normalise to one key, because "10 percent" and "10%" are the same
 * claim about the world and a registry entry sourcing one of them sources both.
 */
export function figureRegex() {
  return /(?:€\s?\d[\d,]*(?:\.\d+)?(?:\s*(?:million|bn|k))?|\d[\d,]*(?:\.\d+)?\s*EUR\b|\d+(?:\.\d+)?\s*(?:%|percent\b)|\d[\d,]*(?:\.\d+)?\s*(?:business\s+)?(?:days?|weeks?|months?|years?))/gi;
}

export function normaliseFigure(match) {
  const s = match.trim().replace(/\s+/g, ' ').toLowerCase();
  const pct = s.match(/^(\d+(?:\.\d+)?)\s*(?:%|percent)$/);
  if (pct) return `${pct[1]}%`;
  const eur = s.match(/^(\d[\d,]*(?:\.\d+)?)\s*eur$/);
  if (eur) return `€${eur[1]}`;
  return s.replace(/^€\s+/, '€');
}

export function figurePhrases(text) {
  return (text.match(figureRegex()) || []).map(normaliseFigure);
}
...
export function buildCorpusIndex(docs) {
  ...
  for (const doc of prepared) {
    ...
    for (const sentence of doc.sentences) {
      for (const sk of sentenceShapes(sentence)) {
        if (seenShapes.has(sk)) continue;
        seenShapes.add(sk);
        if (!shapeOwners.has(sk)) shapeOwners.set(sk, new Set());
        shapeOwners.get(sk).add(doc.id);
      }
    }
  ...
  return { prepared, shingleOwners, shapeOwners, figureCounts, docCount: prepared.length };
}
...
export function analyseDoc(docId, index) {
  ...
  // Shape collisions are reported as a share as well as a count. The count
  // alone is unusable as a penalty here: the machine files carry a median of
  // 110 collisions and the semi-automatic middle carries 24, so any coefficient
  // that punishes the first flattens the second, and the middle is the whole
  // proof that the scale is not binary.
  ...
  for (const sentence of doc.sentences) {
    for (const sk of sentenceShapes(sentence)) {
      ...
      seen.add(sk);
      shapeTotal += 1;
      const owners = index.shapeOwners.get(sk);
  ...
  const templateShapeShare = shapeTotal ? sharedSkeletons.length / shapeTotal : 0;
